import { gauge } from "../gen/messages";
import { HookMethod } from "../models/HookMethod";
import { HookType } from "../models/HookType";
import { HookExecutionProcessor } from "./HookExecutionProcessor";
export declare class StepExecutionStartingProcessor extends HookExecutionProcessor {
    protected hookType: HookType;
    constructor();
    getExecutionInfo(message: gauge.messages.IMessage): gauge.messages.ExecutionInfo;
    protected getApplicableHooks(message: gauge.messages.IMessage): Array<HookMethod>;
}
